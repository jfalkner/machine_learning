...
```bash
conda run -n bayesian_chart python monthly_charting.py
```

Alternatively, you can activate the conda environment.

```bash
conda activate bayesian_chart
```

Run the first example: monthly chart.
```python
 python monthly_charting.py
```

Run the second example: weekly chart.
```python
 python weekly_charting.py
```
...
```bash
conda remove --name bayesian_chart --all
```